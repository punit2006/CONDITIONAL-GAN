...
from .dataset import get_batch, one_hot
from .models import GeneratorBasic, DiscriminatorBasic

def train_basic(epochs=1000):
    generator = GeneratorBasic().to(device)
    discriminator = DiscriminatorBasic().to(device)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            print(f"[Basic] Epoch {epoch} D loss: {d_loss.item():.4f} G loss: {gen_loss.item():.4f}")
